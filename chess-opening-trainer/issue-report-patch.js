...
const ISSUE_APP_VERSION="0.2.0";
let ISSUE_SCREENSHOT_PROMISE=null;
...
function issueLiveGame(){try{return (typeof game!=="undefined"&&game?.fen)?game:(state?.game?.fen?state.game:(state?.chess?.fen?state.chess:(globalThis.game?.fen?globalThis.game:(globalThis.chess?.fen?globalThis.chess:null))))}catch{return null}}
function issueCurrentGameRecord(){let moves=[],fen="Unavailable";try{const g=issueLiveGame();fen=g?.fen?.()||"Unavailable";moves=(g?.history?.({verbose:true})||[]).map((m,i)=>({ply:i+1,side:i%2===0?"white":"black",san:m.san||"",uci:`${m.from||""}${m.to||""}${m.promotion||""}`,from:m.from||"",to:m.to||"",promotion:m.promotion||"",before:m.before||"",after:m.after||""}))}catch{}return{fen,moves}}
function issueContext(){const g=issueLiveGame();return{appVersion:ISSUE_APP_VERSION,buildRef:window.CHESS_BUILD_REF||null,email:state.profileEmail||"",side:state.side||null,level:state.level||null,screen:state.screen||null,mode:state.mode||null,variation:(state.variationIndex??0)+1,userMoves:state.userMoveCount||0,status:state.status||null,currentFen:g?.fen?.()||null,time:new Date().toISOString(),browser:navigator.userAgent,url:location.href,viewport:{width:innerWidth,height:innerHeight,scrollY:scrollY}}}
async function issueLoadHtml2Canvas(){if(globalThis.html2canvas)return globalThis.html2canvas;await new Promise((resolve,reject)=>{const existing=document.querySelector('script[data-issue-html2canvas]');if(existing){existing.addEventListener('load',resolve,{once:true});existing.addEventListener('error',reject,{once:true});return}const s=document.createElement('script');s.src='https://cdn.jsdelivr.net/npm/html2canvas@1.4.1/dist/html2canvas.min.js';s.async=true;s.dataset.issueHtml2canvas='1';s.onload=resolve;s.onerror=reject;document.head.appendChild(s)});return globalThis.html2canvas}
async function issueCaptureScreenshot(){let hidden=[],savedInputs=[];try{const h2c=await issueLoadHtml2Canvas();for(const el of document.querySelectorAll('#cloudAccountBadge,#issueReportModal')){hidden.push([el,el.style.visibility]);el.style.visibility='hidden'}for(const el of document.querySelectorAll('input[type="email"],input[type="password"]')){savedInputs.push([el,el.value]);el.value=''}const canvas=await h2c(document.documentElement,{useCORS:true,allowTaint:false,backgroundColor:'#070b10',x:window.scrollX,y:window.scrollY,width:window.innerWidth,height:window.innerHeight,scale:Math.min(window.devicePixelRatio||1,1.5),logging:false});return await new Promise(resolve=>canvas.toBlob(resolve,'image/jpeg',0.62))}catch{return null}finally{for(const [el,v] of hidden)el.style.visibility=v;for(const [el,v] of savedInputs)el.value=v}}
async function issueUploadScreenshot(blob,signedIn,s){if(!blob)return null;const sb=window.CHESS_SUPABASE;if(!sb?.url||!sb?.key)return null;const id=(globalThis.crypto?.randomUUID?.()||`${Date.now()}-${Math.random().toString(36).slice(2)}`)+'.jpg';const auth=signedIn&&s?.access_token?{Authorization:`Bearer ${s.access_token}`}:{ };const r=await fetch(`${sb.url}/storage/v1/object/issue-screenshots/${id}`,{method:'POST',headers:{apikey:sb.key,...auth,'Content-Type':'image/jpeg','x-upsert':'false'},body:blob});if(!r.ok)return null;return `${sb.url}/storage/v1/object/public/issue-screenshots/${id}`}
