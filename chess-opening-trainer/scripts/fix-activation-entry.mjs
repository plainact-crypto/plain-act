...
if (count('function driveTo(a){') !== 1) throw new Error(`Expected exactly one activation driveTo in final source, found ${count('function driveTo(a){')}`);
...
const replacement = `  globalThis.__COT_ACTIVATION_ENTRY_HOTFIX__='direct-v5';
  ...
  async function driveTo(a){
    ...
    try{render?.()}catch{}
    const launchVisibleVariation=()=>{const cards=[...document.querySelectorAll('.variation-card')].filter(visible);const card=cards[Number(a.variation||0)];if(!card)return false;if(a.mode==='guided'){const button=card.querySelector('[data-new]');if(visible(button)&&!button.disabled){document.documentElement.dataset.cotActivationEntry='fallback-data-new';button.click();return true}}if(a.mode==='test'){const button=[...card.querySelectorAll('button')].find(x=>/Practice Test/i.test(x.textContent||'')&&visible(x)&&!x.disabled);if(button){document.documentElement.dataset.cotActivationEntry='fallback-practice';button.click();return true}}return false};
